...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    ...
    read(fd, &student, STUDENT_RECORD_SIZE);
    if (student.id != 0) {
...
int del_student(int fd, int id) {
    ...
    // Delete the student by writing an empty record
    lseek(fd, offset, SEEK_SET);
...
void print_student(student_t *s) {
    ...
    printf(M_STUDENT_FOUND, s->id, s->fname, s->lname, real_gpa);
}
